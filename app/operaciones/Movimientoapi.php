...
if ($httpCode === 200) {
    ...
    if ($nitDian === '' || $claveCertificadoDian === '' || $softwareIdDian === '' || $llaveEnvioDian === '') {
        http_response_code(500);
        echo json_encode([
            "success" => false,
            ...
            "config" => [
                ...
                "llave_envio" => $llaveEnvioDian !== '' ? "OK" : "FALTA"
            ]
        ]);
        exit;
    }
    ...
    $externalUrldian = "https://www.factin.app:8443/EnvioApi/"
        ...
        . "?llave=" . rawurlencode($llaveEnvioDian);

    $payloadEnviado = $externalData;
    $dianRequestDebug = [
        "endpoint" => "https://www.factin.app:8443/EnvioApi/{prefijo}/{consecutivo}/{nit}/{clave}/{softwareId}",
        "prefijo" => $prefijo,
        "consecutivo" => (string)$consecutivo,
        "nit" => $nitDian,
        "softwareId" => $softwareIdDian,
        "llaveEnvioMasked" => $llaveEnvioDian !== ''
            ? str_repeat('*', max(0, strlen($llaveEnvioDian) - 4)) . substr($llaveEnvioDian, -4)
            : '',
