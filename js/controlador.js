	...
	$scope.guardarFacturaElec = function(insertClientes,cambioFacturaDinero)
	{
		...
		if ($scope.listaProductosDetalleFactura!="" && cambioFacturaDinero.cambio>=0 ) {
			...
			{		
				...
				// var descuentoDividido=cambioFacturaDinero.descuento/$scope.listaProductosDetalleFactura.length;
				var mensajeexito = 0;
				var totalDetalles = $scope.listaProductosDetalleFactura.length;
				var detallesProcesados = 0;
				var huboErrorDetalle = false;

				var finalizarGuardadoDetalles = function () {
					...
					if (huboErrorDetalle) {
						new PNotify({
							title: 'Error',
							text: 'No se guardaron todos los productos de la factura. No se enviará a DIAN.',
							type: 'error',
							styling: 'bootstrap3'
						});
						return;
					...
					if ($scope.tipo_factura_actual == 2) {
						console.log('Llamando a Movimientoapi con detalle ya guardado...');
